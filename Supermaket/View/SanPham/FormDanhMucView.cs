    ...
    public partial class FormDanhMucView : MauView
    {
        ...
        public void LoadData()
        {
            if (connection.State == ConnectionState.Closed)
                connection.Open();
            string sql = @"select ROW_NUMBER() OVER (ORDER BY TENDM) AS 'STT',TENDM,DANHMUC.MADM,DANHMUC.MOTA,TENKM
            from DANHMUC LEFT JOIN SANPHAM ON SANPHAM.MADM = DANHMUC.MADM
                         LEFT JOIN KHUYENMAI ON SANPHAM.MAKM = KHUYENMAI.MAKM
            where TENDM LIKE
            N'%" + txtTk.Text + "%'"+
            "GROUP BY TENDM,DANHMUC.MADM,DANHMUC.MOTA,TENKM";
            ...
            data.Columns[4].HeaderText = "Tên KM";
        ...
        public override void data_DoubleClick(object sender, EventArgs e)
        {
            ...
            themdm.cbxMaKM.Text = data.CurrentRow.Cells["TENKM"].Value.ToString();
            themdm.ShowDialog();
            LoadData();
        }
